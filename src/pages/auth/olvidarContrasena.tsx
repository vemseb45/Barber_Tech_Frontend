...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Petición a tu API en Django para SOLICITAR la recuperación
      const res = await api.post('/recuperacion/solicitar/', { email });

      // Si axios responde bien (200 OK)
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full p-4 pl-12 rounded-2xl bg-slate-100 dark:bg-white/5 border transition-all outline-none text-sm font-medium ${
                      error ? "border-red-500 ring-4 ring-red-500/10" : successMsg ? "border-green-500 ring-4 ring-green-500/10" : "border-transparent focus:border-[#9333ea] focus:ring-4 focus:ring-[#9333ea]/10"
                    }`}
                    value={email}
