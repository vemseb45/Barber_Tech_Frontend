    ...
    const fetchCitas = async () => {
      try {
        ...
        const res = await fetch(
          "http://127.0.0.1:8000/api/cita/terminadas/cliente/",
          {
            headers: {
        ...
        if (!res.ok) {
          throw new Error("Error al obtener citas terminadas");
        }
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm">
          Aquí puedes ver tus citas finalizadas
        </p>
      </div>
        ...
        {citas.map((cita) => (
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <div className="px-4 py-2 rounded-full text-xs font-bold bg-green-100 text-green-600">
                Terminada
              </div>
            </div>
          </motion.div>
